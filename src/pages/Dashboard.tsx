...
export default function DashboardPage() {
  ...
  const pinnedNotes = notes.filter(
    (note: any) => !note.isDeleted && note.isPinned,
  );
  const bookmarkedNotes = notes.filter(
    (note: any) => !note.isDeleted && note.isBookMarked,
  );
  ...
  const recentNotes = activeNotes
    .filter((note: any) => note.lastUpdated)
    .sort((a: any, b: any) => {
      try {
        return (
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const hasNoteOnDate = sortedNotes.some((note: any) => {
        try {
          const noteDate = new Date(note.dateCreated);
          return (
            format(noteDate, "yyyy-MM-dd") === format(checkDate, "yyyy-MM-dd")
          );
        } catch (error) {
          return false;
        }
  ...
  const weeklyProgress =
    weeklyGoal > 0 ? (thisWeekNotes.length / weeklyGoal) * 100 : 0;
    ...
    <Container maxWidth="xl">
      ...
      {(storeError || queryError) && (
        <Alert severity="warning" sx={{ mb: 3 }}>
          Some data might not be up to date. Please refresh if you encounter
          issues.
        </Alert>
      )}
